...
El método más básico es el método del punto fijo, que es un método explícito,
y consiste en tomar como semilla $\bar{x}_n$ para cada iteración $n$ a la solución del paso previo $\bar{F} \left( \bar{x}_{n-1} \right)$.
...
Otro métodos \textit{quasi-Newton} muy utilizados son el método de la secante (\textit{chord method})\cite{kelley} y el método de \textit{Shamanskii}.
...
Sin embargo, en cada paso de iteración de resolución del sistema no lineal ({outer iteration}) requieren múltiples iteraciones del método de \textit{Krylov} (\textit{inner iterations}),
