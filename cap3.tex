...
La distribución espacial de potencia, a su vez, realimenta las secciones eficaces,
ya sea a que está actuando como una fuente de energía, modificando temperaturas y densidades de combustibles, refrigerantes y demás materiales,
...
En la sección \ref{3:strategy-extended} se genera una estrategia de acoplamiento que permite utilizar técnicas explícitas e implícitas.
En las secciones \ref{3:relap-fermi} y \ref{3:relap-puma} se presentan dos ejemplos de acoplamiento neutrónico-termohidráulico utilizando esta estrategia.
...
El cálculo de núcleo puede efecutarse mediante un modelo de difusión estacionario
\footnote{Al modelar un flujo estacionario en realidad se está calculando el flujo en un reactor crítico asociado,
por lo que la distribución de potencia hallada solo será válida si el $k_{eff}$ calculado es 1.
En caso contrario, debería repetirse el cálculo considerando otra distribución de secciones eficaces
...
\label{eq-nt}
\end{equation}
donde $\bar{\phi}$ es un vector con el valor del flujo neutrónico a diferentes grupos de energía integrado en diferentes celdas de cálculo del reactor,
$\Sigma$ es la matriz de secciones eficaces que contiene valores asociados a diferentes reacciones para diferentes grupos de energía y en las diferentes celdas de cálculo,
y $k_{eff}$ es el factor de criticidad del reactor.
Si bien las secciones eficaces tienen diversas dependencias,
en el modelo propuesto solo dependen de la densidad del refrigerante $N_{ref}$, de la temperatura del refrigerante $T_{ref}$ y de la temperatura del combustible $T_{comb}$ , de modo que:

\begin{equation}
\Sigma = \Sigma \left ( N_{ref}, T_{ref}, T_{comb} \right )
\label{eq-sigma}
\end{equation}
El valor de la potencia $P_i$ en cada celda de cálculo $i$ puede modelarse en función del flujo neutrónico obtenido en esa celda \cite{lamarsh} de la siguiente manera:

\begin{equation}
P_i = \sum_j E_{fis,j} \Sigma_{fis,j} \phi_{i,j}
\label{power}
\end{equation}
donde $E_{fis,j}$ es la energía liberada por fisión en el grupo de energía $j$,
$\Sigma_{fis,j}$ es la sección eficaz de fisión condensada en el grupo de energía $j$ y
$\phi_{i,j}$ es la componente del flujo neutrónico en el grupo de energía $j$.
La distribución de potencia $\bar{P}$ hallada es utilizada como fuente de energía en el balance termohidráulico.



\subsection*{Acoplamiento neutrónico-termohidráulico utilizando RELAP y FERMI}
\label{3:relap-fermi}
