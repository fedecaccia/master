...
Por simplicidad, en este trabajo se utilizan condiciones de borde homogéneas\footnote{
...
En el primer ejemplo analizado,
la ecuación que modela el flujo neutrónico es resuelta con \textbf{Fermi} \cite{fermi}.
...
considerando un único canal de refrigeración con longitud que recibe una potencia igual a la potencia total generada por el reactor.
...
\caption[Esquema de núcleo simple en análisis de acoplamiento neutrónico-termohidráulico utilizando \textit{RELAP5} y \textit{Fermi}]
{Esquema de núcleo simple en análisis de acoplamiento neutrónico-termohidráulico utilizando \textit{RELAP5} y \textit{Fermi}.}
\label{fuel_z5}
\end{figure}
...
Por este motivo se construyeron funciones de $\Sigma$ dependientes en $T_{comb}$, $T_{ref}$ y $N_{ref}$
...
Se definen cinco zonas axiales para el cálculo termohidráulico.
Las temperaturas y densidades medias obtenidas en cada una de ellas definen las secciones eficaces para el cálculo neutrónico en la zona correspondiente.
...
\label{3:relap-puma}
En el segundo ejemplo analizado, la ecuación que modela el flujo neutrónico es resuelta con \textbf{PUMA} \cite{puma} a cinco grupos de energía.
...
Las ecuaciones hidrodinámicas se resuelven con \textbf{RELAP5}.

El dominio de cálculo nautrónico consiste en un modelo del núcleo del reactor CAREM-25 proporcionado por DIFRA.
